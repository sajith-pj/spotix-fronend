...
class BlackNavbar extends Component {
  ...
  handleScroll = () => {
    if (window.scrollY > 200) {
      document.querySelector(".d-nav").classList.add("bg-white");
    } else {
      document.querySelector(".d-nav").classList.remove("bg-white");
    }
  };
      ...
      <nav className="nav w-full fixed bg-transparent d-nav top-0 md:px-10 px-2 md:py-3 py-2">
        <div className="mx-auto flex flex-wrap items-center justify-between">
          ...
          <label
            ...
            htmlFor="menu-btn"
          >
            <span className="black-navicon bg-white flex items-center relative" />
          </label>
          <ul className="menu md:bg-transparent bg-blend-darken md:border-none flex justify-end list-reset ml-auto w-full md:w-auto">
            <li className="border-none">
              <Link href="/" >
                <a className="nav-link-color block md:inline-block px-4 py-3 no-underline text-white hover:text-grey-darker transition ease-linear">
                  Home
                </a>
              </Link>
            </li>
